 ...
 */
public final class RamUsageEstimatorCustom {
  /**
  ...
  /** No instantiation. */
  private RamUsageEstimatorCustom() {}
  ...
  private static long measureSizeOf(ArrayList<Object> stack) {
    ...
    while (!stack.isEmpty()) {
      ...
      if (obClazz.isArray()) {
         ...
         */
        long size = NUM_BYTES_ARRAY_HEADER;
        System.out.println("Array-->header:" + size);
  ...
  private static ClassCache createCacheEntry(final Class<?> clazz) {
    ...
    System.out.println("header: " + shallowInstanceSize);
    ...
    for (Class<?> c = clazz; c != null; c = c.getSuperclass()) {
      ...
      for (final Field f : fields) {
        if (!Modifier.isStatic(f.getModifiers())) {
          ...
          System.out.println("add " + f.getName() + ": " + shallowInstanceSize);
    ...
    System.out.println(clazz.getSimpleName() + " total(add align pending) : " + cachedInfo.alignedShallowInstanceSize);
    return cachedInfo;
  }
